...
static Ret  ftk_input_method_gpinyin_init(FtkInputMethod* thiz)
{
	...
	ftk_snprintf(dict_filename, FTK_MAX_PATH, "%s/gpinyin.dat", DATA_DIR);
	if(!ftk_file_exist(dict_filename))
	{
		ftk_snprintf(dict_filename, FTK_MAX_PATH, "%s/gpinyin.dat", LOCAL_DATA_DIR);
		ftk_snprintf(user_dict_filename, FTK_MAX_PATH, "%s/gpinyin_user.dat", LOCAL_DATA_DIR);
	}
	else
	{
		ftk_snprintf(user_dict_filename, FTK_MAX_PATH, "%s/gpinyin_user.dat", DATA_DIR);
	}
